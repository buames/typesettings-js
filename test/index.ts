...
test('generateFontFace()', (t) => {
  const a = create();
  t.snapshot(a.fontFace, 'Returns a string with the family wrapped in quotes');

  const b = create({ family: 'Helvetica' });
  t.snapshot(b.fontFace, 'Returns a string without the family wrapped in quotes');

  const c = create({ fontFaceStyles: { fontDisplay: 'swap', WebkitFontSmoothing: 'antialiased' } });
  t.snapshot(c.fontFace, 'Returns a string with additional styles');

  const d = create({ eot: false, woff: false, woff2: false, ttf: false });
  t.snapshot(d.fontFace, 'Returns a string with only local sources');
});

test('generateFonts()', (t) => {
  const a = create();
  t.snapshot(a.fonts, 'Returns a fonts object with a fallback family');

  const b = create({ fallbacks: undefined });
  t.snapshot(b.fonts, 'Returns a fonts object without a fallback family');

  const c = create({ fontStyles: { textRendering: 'optimizeLegibility' } });
  t.snapshot(c.fonts, 'Returns a fonts object with additional styles');
});
...
test('Should return font-family values', (t) => {
  const a = create();
  t.is(
    a.fonts.s20.n500.fontFamily,
    '\'Helvetica Neue\', -apple-system,BlinkMacSystemFont',
    'Should return font-family with quotes'
  );

  const b = create({ family: 'Helvetica' });
  t.is(
    b.fonts.s20.n500.fontFamily,
    'Helvetica, -apple-system,BlinkMacSystemFont',
    'Should return font-family without quotes'
  );

  const c = create({ fallbacks: undefined });
  t.is(
    c.fonts.s20.n500.fontFamily,
    '\'Helvetica Neue\'',
    'Should return font-family without a fallback'
  );
});
...
test('Should return font-weight values', (t) => {
  const a = create();
  t.is(a.fonts.s20.n500.fontWeight, 500, 'Should return medium as a number');
  t.is(a.fonts.s20.nBold.fontWeight, 'bold', 'Should return bold as a string');
});

test('Should return letter-spacing values', (t) => {
  const a = create();
  t.is(a.fonts.s14.n400.letterSpacing, null, 'Should return null');
  t.is(a.fonts.s20.n500.letterSpacing, '0.29px', 'Should return in px');
  t.is(a.fonts.s20.nBold.letterSpacing, '0.29em', 'Should return in em');
});

test('Should return line-height values', (t) => {
  const a = create();
  t.is(a.fonts.s20.n500.lineHeight, '22px', 'Should return line-height in px');
  t.is(a.fonts.s20.nBold.lineHeight, '22em', 'Should return in em');
...
test('Should return text-transform values', (t) => {
  const a = create();
  t.is(a.fonts.s20.n500.textTransform, 'none', 'Should return none');
  t.is(a.fonts.s20.n500_caps.textTransform, 'uppercase', 'Should return uppercase');
  t.is(a.fonts.s20.n500_lower.textTransform, 'lowercase', 'Should return lowercase');
});
